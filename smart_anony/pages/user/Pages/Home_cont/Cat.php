<?php
//use function UI\quit;
include_once("../../../connection.php");
?>
<!DOCTYPE html>
<html>
    <head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible " content="IE=edge">
<title></title>
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/css/bootstrap.min.css">
<link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">

<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/js/bootstrap.min.js"></script>


<link rel="stylesheet" type="text/css" href="../../CSS/External.css">
<style>
		a,h1,p.con,pre {
		color:white;
		font-family:Arial;
		text-decoration:none;
		}
	</style>

</head>
<body style="background-color:black">
      <div class="container mt-5">
            <div class ="row">
                <?php
                $query="SELECT * FROM product";
                $result=$conn->query($query);
                if($result->num_rows > 0)
                {
while($rows = $result->fetch_assoc()){
$name=$rows['Prod_Name'];
$qty=$rows['Qty_Availble'];
$price=$rows['List_Price'];
$image=$rows['file_name'];
$des=$rows['Discription'];

?>
<div class="col-sm-12 col-md-4 col-lg-4">
    <div class="card border-0">

    
       <!-- //class="card-img"
    //echo '<img src="data:image;base64,'.base64_encode($image).'" style=""height:100px">';
-->
    <center><img src="../../../../templates/uploads/<?php echo $rows['file_name'];?>" width="280" style="margin-top:20px;"></center>
    <br>
        <div class="card-body">
            <h5 class="card-title text-secondary">Item:  <?php echo $name; ?></h5>
            <h6 class="card-title">Available : <?php echo $qty; ?></h6>
            <h4 class="card-title text-success"> Rs. <?php echo $price; ?></h4>
            <h6 class="card-title">Description : <?php echo $des; ?></h6>
            <!--<div class="buy d-flex justify-content-between align-items-center">
                <a href="index.php?id=<?php echo $row['id']; ?>" class="btn btn-danger mt-3"><i class="fas fa-shopping-cart"></i>ADD TO CART</a>
            </div>-->
            <br><br>

            <div class="buy d-flex justify-content-between align-items-center" style="margin-left:30%;">
            <a href="#" class="btn btn-danger mt-3">ADD TO CART</a>
            </div>
        </div>
    </div>
</div>

<?php }} ?>
            </div>
                
        
</body>
</html>


