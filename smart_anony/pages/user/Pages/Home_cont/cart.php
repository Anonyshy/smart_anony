...
$query2 = "SELECT * FROM customer WHERE Username='$loginame' LIMIT 1";
...
<body background="../../Images/bg_prod.jpg">
	...
	<div class="container mt-5">
		...
		<?php
		//=================================find last order id=========================================
		$q = "SELECT Order_ID FROM order_tbl ORDER BY Order_ID DESC LIMIT 1";
		$r = $conn->query($q);
		if ($r->num_rows > 0) {
			$rw = $r->fetch_assoc();
			$oidStr = $rw['Order_ID'];
		...
		<script>
			...
			payhere.onCompleted = function onCompleted(orderId) {
				...
				window.location.href = "invoice_process.php?id=<?= $id ?>&name=<?= $loginame ?>&pn=<?= $pn ?>&address=<?= $address ?>&email=<?= $email ?>&oid=<?= $oid ?>&total=<?= $total ?>";
